...
(7d)" should show Facebook, Instagram (Wed, via Buffer), LinkedIn (Thu) and
GBP (Tue). A live channel at zero for two weeks is dead, not quiet. Graph
publish for Instagram stays off.
...
- **Token check** on /admin → Channel health. LinkedIn expires ~every 60 days.
  ...
  token env vars unused; do not turn `IG_AUTOPOST` back to `live`.
